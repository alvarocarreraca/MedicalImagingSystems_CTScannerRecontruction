function [img,H] = iradon1(varargin)
...
[p,theta,filter,d,interp,N] = parse_inputs(args{:}); disp('119')

[p,H] = filterProjections(p, filter, d); disp('121')
...
imgDiag = 2*ceil(N/sqrt(2))+1; disp('139')  % largest distance through image. 
if size(p,1) < imgDiag
    rz = imgDiag - size(p,1);  % how many rows of zeros
    p = [zeros(ceil(rz/2),size(p,2)); p; zeros(floor(rz/2),size(p,2))]; disp('142')
...
switch interp
    case 'nearest neighbor'
        ...
        img = builtin("_iradonmex", N, theta, x, y, p, 0); disp('150')
        
    case 'linear'
        
        img = builtin("_iradonmex", N, theta, x, y, p, 1); disp('154')
        
    case {'spline','pchip','cubic','v5cubic'}
        
        interp_method = sprintf('*%s',interp); disp('158') % Add asterisk to assert
                                               % even-spacing of taxis
        
        % Generate trigonometric tables
        costheta = cos(theta);
        sintheta = sin(theta); disp('163')
        
        % Allocate memory for the image
        img = zeros(N,'like',p);      disp('166')   
...
img = img*pi/(2*length(theta)); disp('178')
...
% Design the filter
len = size(p,1); disp('186')
H = designFilter(filter, len, d); disp('187')
...
p = fft(p);   disp('199')             % p holds fft of projections

p = bsxfun(@times, p, H); disp('201') % faster than for-loop

p = ifft(p,'symmetric');  disp('203') % p is the filtered projections

p(len+1:end,:) = [];   disp('205')    % Truncate the filtered projections
%----------------------------------------------------------------------
...
order = max(64,2^nextpow2(2*len)); disp('221')
...
w = 2*pi*(0:size(filt,2)-1)/order;  disp('239')  % frequency axis up to Nyquist
...
switch filter
    ...
    case 'shepp-logan'
        ...
        filt(2:end) = filt(2:end) .* (sin(w(2:end)/(2*d))./(w(2:end)/(2*d))); disp('246')
    case 'cosine'
        filt(2:end) = filt(2:end) .* cos(w(2:end)/(2*d)); disp('248')
    case 'hamming'
        filt(2:end) = filt(2:end) .* (.54 + .46 * cos(w(2:end)/d)); disp('250')
    case 'hann'
        filt(2:end) = filt(2:end) .*(1+cos(w(2:end)./d)) / 2; disp('252')
    otherwise
        error(message('images:iradon:invalidFilter'))
end

filt(w>pi*d) = 0;          disp('257')             % Crop the frequency response
filt = [filt' ; filt(end-1:-1:2)'];  disp('258')   % Symmetry of the filter
%----------------------------------------------------------------------
